...
def check_admin_login(request):
    if request.COOKIES.get('password') == hash_string('admin') and request.COOKIES.get('user_type') == hash_string(
            'admin'):
        return True
    else:
        return False
...
def create_login_cookies(response, email, hashed_password, non_hashed_user_type):
    ...
    response.set_cookie(key="password", value=hashed_password,
                        max_age=max_age, expires=expires)
    response.set_cookie(key="user_type", value=hashed_user_type,
                        max_age=max_age, expires=expires)
...
def download_file(request):
    ...
    if file_type == "abstract":
        path_to_file = os.path.join(
            settings.MEDIA_ROOT, "abstracts\\", file_name)
    else:
        path_to_file = os.path.join(settings.MEDIA_ROOT, "papers\\", file_name)
...
def sign_up(request):
    islogged_in = check_login(request)
    is_admin_logged_in = check_admin_login(request)
    return render(request, "sign_up.html", {"islogged_in": islogged_in, "is_admin_logged_in": is_admin_logged_in,
                                            "user_type": request.COOKIES.get('user_type')})
...
def sign_up_handle(request):
    ...
    if request.method == "POST":
        ...
        context = {"islogged_in": False, "is_admin_logged_in": is_admin_logged_in,
                   "user_type": request.COOKIES.get('user_type')}
        if user_type == "author":
            context['islogged_in'] = True
            user = models.User(
                email=email, password=hashed_password, type="author")
            user.save()
            ...
            if user_type == "chair":
                is_chair = True
                user = models.User(
                    email=email, password=hashed_password, type="chair")
                user.save()
                chair = models.Chair(
                    email=email, password=hashed_password, name=name)
                chair.save()
                ...
                is_chair = False
                user = models.User(
                    email=email, password=hashed_password, type="reviewer")
                user.save()
...
def login(request):
    ...
    return render(request, "login.html",
                  {"islogged_in": islogged_in, 'message': "", "is_admin_logged_in": is_admin_logged_in,
                   "user_type": request.COOKIES.get('user_type')})
...
def login_handle(request):
    ...
    if request.method == "POST":
        ...
        if email == 'admin' and password == 'admin':
            ...
            response = render(
                request, "chair_reviewer_application.html", context)
            hashed_admin_password = hash_string('admin')
            create_login_cookies(
                response, 'admin', hashed_admin_password, 'admin')
            return response
        else:
            ...
            try:
                ...
                user_type = user.type
                try:
                    hashed_user_type = hashlib.sha224(
                        user_type.encode('utf-8')).hexdigest()
                    context = {"islogged_in": True, "type_name": user_type,
                               "user_type": hash_string(user_type)}
                    ...
                    response.set_cookie(
                        key="email", value=email, max_age=max_age, expires=expires)
                    if user_type == "chair":
                        chair = models.Chair.objects.get(
                            email=email, password=hashed_password)
                        response.set_cookie(
                            key="password", value=chair.password, max_age=max_age, expires=expires)
                        response.set_cookie(
                            key="user_type", value=hashed_user_type, max_age=max_age, expires=expires)

                    elif user_type == "reviewer":
                        reviewer = models.Reviewer.objects.get(
                            email=email, password=hashed_password)
                        response.set_cookie(
                            key="password", value=reviewer.password, max_age=max_age, expires=expires)
                        response.set_cookie(
                            key="user_type", value=hashed_user_type, max_age=max_age, expires=expires)
                        ...
                        author = models.Author.objects.get(
                            email=email, password=hashed_password)
                        response.set_cookie(
                            key="password", value=author.password, max_age=max_age, expires=expires)
                        response.set_cookie(
                            key="user_type", value=hashed_user_type, max_age=max_age, expires=expires)
                    return response
                ...
                # Non existing user
                print(e)
                return render(request, "login.html", {"islogged_in": True, 'message': 'Bad Authentication.',
                                                      "is_admin_logged_in": is_admin_logged_in, "user_type": request.COOKIES.get('user_type')})
...
def logout_handle(request):
    ...
    response = render(request, "login.html", {
                      "islogged_in": False, "is_admin_logged_in": False})
...
def conferences(request):
    ...
    if request.COOKIES.get('user_type') == hashed_user_type:
        ...
        chair = models.Chair.objects.get(email=request.COOKIES.get(
            'email'), password=request.COOKIES.get('password'))
...
def add_conference_handle(request):
    if request.method == "POST":
        ...
        abstract_submission_deadline = datetime.datetime.strptime(request.POST.get('abstract_submission_deadline'),
                                                                  '%Y-%m-%dT%H:%M')
        paper_submission_deadline = datetime.datetime.strptime(request.POST.get('paper_submission_deadline'),
                                                               '%Y-%m-%dT%H:%M')
        ...
        chair = models.Chair.objects.get(email=request.COOKIES.get(
            'email'), password=request.COOKIES.get('password'))
        conference = models.Conference(name=name, start_date=start_date, end_date=end_date,
...
def schedule_conference(request):
    if request.method == "GET":
        ...
        if not islogged_in:
            ...
            submits = models.Submits.objects.filter(conference=conference,
                                                    submit_status=constants.PAPERSUBMIT_PAPER_ACCEPTED)
            ...
            for schedule in schedules:
                ...
                scheduled_papers.append(
                    [None, last_schedule_time - datetime.timedelta(minutes=15), last_schedule_time])
...
def unschedule_paper_handle(request):
    if request.method == "GET":
        ...
        if not islogged_in:
            ...
            chair = models.Chair.objects.get(email=request.COOKIES.get('email'),
                                             password=request.COOKIES.get('password'))
            schedule = models.Schedules.objects.get(
                conference=conference, paper=paper, chair=chair)
            schedule.delete()
            submit_paper.save()
            return redirect('/schedule_conference?conf_name=' + conf_name)
...
def conference(request):
    if request.method == "GET":
        islogged_in = check_login(request)
        is_admin_logged_in = check_admin_login(request)
        conference = models.Conference.objects.get(
            name=request.GET.get('conf_name'))
        ...
        if request.COOKIES.get('user_type') == author_hashed_user_type:
            ...
            if conference.abstract_submission_deadline > timezone.now():
                ...
                papers = models.Paper.objects.filter(
                    Q(author=author), Q(status=constants.PAPAERSUBMIT_INITIAL))
                submits = models.Submits.objects.filter(Q(author=author),
                                                        Q(submit_status=constants.PAPERSUBMIT_ABSTRACT_REJECTED) |
        ...
        return render(request, "conference.html",
                      {"islogged_in": islogged_in, "submits": submits_conf2,
                       "user_type": request.COOKIES.get('user_type'),
                       "author_hashed_user_type": author_hashed_user_type,
                       "chair_hashed_user_type": chair_hashed_user_type, "conf_name": request.GET.get('conf_name'),
                       "is_admin_logged_in": is_admin_logged_in,
    ...
    if request.method == "GET":
        ...
        if is_admin_logged_in:
            ...
            response = render(request, "chair_reviewer_application.html",
                              {"islogged_in": True, "is_admin_logged_in": True, "chairs": chairs, "reviewers": reviewers, "hashed_user_type": request.COOKIES.get('user_type'),
                               "user_type": request.COOKIES.get('user_type')})
            return response
...
def chair_reviewer_application(request):
    if request.method == "GET":
        ...
        if is_admin_logged_in:
            ...
            reviewers = models.Reviewer.objects.all()
            response = render(request, "chair_reviewer_application.html",
                              {"islogged_in": True, "is_admin_logged_in": is_admin_logged_in, "chairs": chairs,
                               "reviewers": reviewers, "user_type": request.COOKIES.get('user_type')})
            return response
    ...
    if request.method == "GET":
        ...
        if not is_logged_in:
            ...
            response = render(request, "author_papers.html",
                              {"islogged_in": is_logged_in, "papers_submits": papers_submits, "user_type": request.COOKIES.get('user_type'),
                               "pending": constants.PAPERSUBMIT_PAPER_PENDING + constants.PAPERSUBMIT_ABSTRACT_PENDING,
                               "accepted": constants.PAPERSUBMIT_PAPER_ACCEPTED + constants.PAPERSUBMIT_ABSTRACT_ACCEPTED,
                               "rejected": constants.PAPERSUBMIT_ABSTRACT_REJECTED + constants.PAPERSUBMIT_PAPER_REJECTED})
            return response
...
def add_paper_handle(request):
    ...
    if not islogged_in:
        ...
        if request.method == 'POST' and request.FILES['abstract_file']:
            ...
            abstract_directory = os.path.join(
                settings.MEDIA_ROOT, 'abstracts\\')
            if not os.path.exists(abstract_directory):
                os.makedirs(abstract_directory)

            full_filename = os.path.join(
                abstract_directory, file_abstract.name)
            fout = open(full_filename, 'wb+')

            file_content = ContentFile(request.FILES['abstract_file'].read())

            # Iterate through the chunks.
            for chunk in file_content.chunks():
                fout.write(chunk)
            fout.close()
            file_abstract_url = file_abstract.name
            ...
            response = render(request, "author_papers.html",
                              {"islogged_in": islogged_in, "papers_submits": papers_submits,
                               "user_type": hash_string('author'), "user_type": request.COOKIES.get('user_type')})
            return response
    ...
    if request.method == "POST":
        ...
        if not islogged_in:
            ...
            if str(paper.status) != constants.PAPAERSUBMIT_INITIAL:
                ...
                submit = models.Submits(
                    conference=conference, author=paper.author, paper=paper)
                submit.save()
...
def assign_paper(request):
    if request.method == "GET":
        ...
        if not islogged_in:
            ...
            paper = models.Paper.objects.get(
                id=int(request.GET.get('paper_id')))
            reviewers_assigns = []
            for reviewer in reviewers:
                try:
                    assign = models.Assigns.objects.get(
                        paper=paper, reviewer=reviewer)
                    print(assign)
                    reviewers_assigns.append([reviewer, assign])
                except:
                    reviewers_assigns.append([reviewer, None])
        return render(request, "assign_paper.html",
                      {"islogged_in": islogged_in, "is_admin_logged_in": is_admin_logged_in,
                       "user_type": request.COOKIES.get('user_type'),
                       "reviewers_assigns": reviewers_assigns, "paper_name": paper.name,
    ...
    if request.method == "GET":
        ...
        if not islogged_in:
            ...
            chair = models.Chair.objects.get(email=request.COOKIES.get('email'),
                                             password=request.COOKIES.get('password'))
            reviewer = models.Reviewer.objects.get(
                email=request.GET.get('reviewer_email'))
            paper = models.Paper.objects.get(
                id=int(request.GET.get('paper_id')))
            conference = models.Conference.objects.get(
                name=request.GET.get('conf_name'))
            assign = models.Assigns(
                chair=chair, reviewer=reviewer, paper=paper, conference=conference)
            assign.save()
            email_send_thread = Thread(target=Utils.send_email, args=(
                "Your review assignment", "You have been assigned paper name : " +
                paper.name + " for a review.",
        ...
        return render(request, "assign_paper.html",
                      {"islogged_in": islogged_in, "is_admin_logged_in": is_admin_logged_in,
                       ...
                       "paper_id": request.GET.get('paper_id'), "paper_name": paper.name,
                       "conf_name": request.GET.get('conf_name')})
    ...
    if request.method == "GET":
        ...
        if not islogged_in:
            ...
            for assignment in assignments:
                ...
                try:
                    review = models.Reviews.objects.get(
                        paper=assignment.paper, reviewer=reviewer)

                    assignments_reviews.append(
                        [assignment, review, is_paper_uploaded])
                    print(is_paper_uploaded)
                except Exception as e:
                    print(e)
                    assignments_reviews.append(
                        [assignment, None, is_paper_uploaded])
...
def add_review_handle(request):
    if request.method == "POST":
        ...
        if not islogged_in:
            ...
            paper = models.Paper.objects.get(
                id=int(request.POST.get('paper_id')))
            ...
            if request.POST.get('is_paper') == "True":
                review = models.Reviews.objects.get(
                    id=int(request.POST.get('review_id')))
    ...
    if request.method == "GET":
        ...
        if not islogged_in:
            ...
            try:
                ...
                print("none")
            review = models.Reviews.objects.get(
                id=int(request.GET.get('review_id')))
            return render(request, "add_review.html",
                          {"islogged_in": islogged_in, "is_admin_logged_in": is_admin_logged_in,
                           "user_type": request.COOKIES.get('user_type'),
                           "paper_id": request.GET.get('paper_id'),
...
def paper_reviews(request):
    if request.method == "GET":
        ...
        if not islogged_in:
            ...
            submit = models.Submits.objects.filter(paper=paper)[0]
            return render(request, "paper_reviews.html",
                          {"islogged_in": islogged_in, "is_admin_logged_in": is_admin_logged_in,
                           "user_type": request.COOKIES.get('user_type'), "paper_name": paper.name, "conf_name": submit.conference.name,
                           "reviews": reviews})
...
def paper_review(request):
    if request.method == "GET":
        ...
        if not islogged_in:
            ...
            review = models.Reviews.objects.get(
                id=int(request.GET.get('review_id')))
            paper = models.Paper.objects.get(id=review.paper.id)
            submit = models.Submits.objects.filter(paper=paper)[0]
...
def accept_reject_paper(request):
    if request.method == "GET":
        ...
        if not islogged_in:
            ...
            is_accepted = request.GET.get('is_accepted')
            submit = models.Submits.objects.get(
                id=int(request.GET.get('submit_id')))
            ...
            if is_accepted == "True":
                ...
                if submit.submit_status == constants.PAPERSUBMIT_ABSTRACT_PENDING or submit.submit_status == constants.PAPERSUBMIT_ABSTRACT_ACCEPTED:
                    ...
                    type = "paper"
            submit.save()
            subject = "Update for your paper : " + submit.paper.name + \
                " in conference " + submit.conference.name + "."
            message = "Your " + type + " for " + submit.paper.name + \
                " has been " + accept_status + ".\n"
            if len(reviews) == 0:
        ...
        response = redirect('/conference?conf_name=' +
                            request.GET.get('conf_name'))
        return response
...
def update_paper_files(request):
    if request.method == "POST":
        ...
        if not islogged_in:
            ...
            paper = models.Paper.objects.get(
                id=int(request.POST.get('paper_id')))
            paper_directory = os.path.join(settings.MEDIA_ROOT, 'papers\\')
            abstract_directory = os.path.join(
                settings.MEDIA_ROOT, 'abstracts\\')
            if 'abstract_file' in request.FILES:
                os.remove(abstract_directory + paper.abstract_file_path)
                file_abstract = request.FILES['abstract_file']
                file_content = ContentFile(file_abstract.read())
                full_filename = os.path.join(
                    abstract_directory, file_abstract.name)
                file_abstract_url = file_abstract.name
                ...
                if paper.paper_file_path:
                    os.remove(paper_directory + paper.name)

                full_filename = os.path.join(paper_directory, paper.name)
                fout = open(full_filename, 'wb+')
                file_content = ContentFile(request.FILES['paper_file'].read())
                ...
                '''
                fs = FileSystemStorage(location=paper_directory)
                # file_abstract_url = fs.url(full_filename)
                file_abstract_url = paper.name
                file_paper_url = paper.name
            ...
            for assign in assigns:
                Utils.send_email_with_thread("About your review assignment",
                                             "Your review assignment for paper : " + paper.name +
                                             " has uploaded a new version of their paper",
